const correlation = require('../src/lc/correlation-constant').prebuilt;
const LongCountFactory = require('../src/factory/long-count');

class MockDateCorrelation {
    constructor(lc, gregorian, julian, jday, maya_day) {
        this.lc = lc;
        this.gregorian = gregorian;
        this.julian = julian;
        this.jday = jday;
        this.maya_day = maya_day;
    }
}

describe('long-count to gregorian/julian', () => {
    const corr = correlation.get(584283);
    const lc_factory = new LongCountFactory();
    const dates = [
        ...
        // ['11.18.3.9.18', '15/10/1582 CE', '5/10/1582 CE', 2299161, 1714878], // Julian to Gregorian Switch
        ['11.18.3.9.17', '14/10/1582 CE', '4/10/1582 CE', 2299160, 1714877], // Julian to Gregorian Switch
        ...
        // ['11.13.19.13.8', '9/3/1500 CE', '28/3/1500 CE', 2268991, 1684708],
        ...
        // ['11.8.18.5.4', '9/3/1400 CE', '28/3/1400 CE', 2232467, 1648184],
        ...
        // ['6.0.0.0.0', '2/28/748 BCE', '8/3/748 BCE', 1448283, 864000],
    ].map(args => new MockDateCorrelation(...args));

    test.each(dates.map(d => [d.lc, d.gregorian, d.jday]))('lc(%s) -> g(%s: %s)', (lc_raw, gregorian) => {
        let lc = lc_factory.parse(lc_raw).set_correlation_constant(corr);
        expect(`${lc.gregorian_date}`).toBe(gregorian);
    });

    test.each(dates.map(d => [d.lc, d.julian]))('lc(%s) -> j(%s)', (lc_raw, julian) => {
        let lc = lc_factory.parse(lc_raw).set_correlation_constant(corr);
        expect(`${lc.julian_date}`).toBe(julian);
    });

    test.each(dates.map(d => [d.lc, d.jday]))('lc(%s) -> jday(%s)', (lc_raw, jday) => {
        let lc = lc_factory.parse(lc_raw).set_correlation_constant(corr);
        expect(lc.julian_day).toBe(jday);
    });

    test.each(dates.map(d => [d.lc, d.maya_day]))('lc(%s) -> maya_day(%s)', (lc_raw, maya_number) => {
        let lc = lc_factory.parse(lc_raw).set_correlation_constant(corr);
        expect(lc.get_position()).toBe(maya_number);
    });
});